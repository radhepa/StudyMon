...
function renderSideQuests(){
  ...
  var h='<div class="section-intro"><span class="eyebrow">THE C-MON QUEST BOARD</span><h2>Side Quests</h2><p>Write real C programs for the people around the region. Try one a week, or work at your own pace. Every quest is available now.</p></div>'+
    ...
    '<details class="panel"><summary>How labs and rewards work</summary><p>These are original labs aligned to the fourth-edition C curriculum (ISBN 9780357506134), not copied textbook exercises. The primary chapter and prerequisites appear on every job.</p><p>Write C here, use Run to try an input, and Submit to compile and check every test. Output must match exactly. Every required test must pass before a quest counts or grants its reward.</p><p>Drafts save in C-MON. There are no external files to import or export. Programs run inside a browser sandbox with a fresh in-memory filesystem for each test.</p></details>'+
...
function renderSideQuest(){
 ...
 '<section class="panel sq-workspace"><div class="sq-editor-head"><h3>Your C program</h3><span class="small">C11 · Clang · Strict warnings</span></div><label class="sr-only" for="sq-source">C source</label><textarea class="sq-editor" id="sq-source" rows="20" spellcheck="false" autocapitalize="off" autocomplete="off" maxlength="64000">'+esc(p.draft)+'</textarea><p id="sq-save-status" class="small" role="status">Draft saved in C-MON. '+(q.grading.mode==='functions'?'Implement the required functions; the game supplies main().':'Write your complete program, including main().')+'</p><div class="sq-runbar"><button id="sq-run" onclick="runQuestCode(false)">Run</button><button class="primary" id="sq-submit" onclick="runQuestCode(true)">Submit</button><button id="sq-stop" class="ghost" onclick="cancelCJob(\'Stopped. Your draft is saved.\')" hidden>Stop</button><span id="sq-job-status" role="status" aria-live="polite">'+(p.status==='completed'?'All tests passed.':'')+'</span></div></section>'+

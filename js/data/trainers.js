...
/* Stable social IDs travel with their source records when arrays are reordered.
   They are data identity only; the prose-bearing array shape stays unchanged. */
...
window.TRAINERS.forEach(function (trainer) {
  ['events', 'outings'].forEach(function (key) {
    var kind = key === 'events' ? 'event' : 'outing';
    (trainer[key] || []).forEach(function (scene) {
      scene.id = trainer.id + '-' + kind + '-' + socialIdPart(scene[0]);
      scene.beatId = scene.id + '-beat-opening';
      labelSocialChoices(scene[3], scene.beatId);
    });
  });
});

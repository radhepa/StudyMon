/* Phase 5 Slice 6: third Tier 2 relationship/location batch - Burl and
   Crag, the Stack Ridge hikers. Both carved out of the shared 'Hiker' cls
   into their own dedicated archetypes ('ballast' / 'stride'), same Tier 2
   shape as sci-oak-phase5.js, oz-phase5.js/sal-phase5.js, barista-phase5.js/
   dax-phase5.js and ace1-phase5.js/ace2-phase5.js: a 150-line target each,
   Tier 2-scaled category minimums, and two active heart events each. Their
   arcs are written to complete each other through ordinary narrative
   reference (each mentions the other by name and history), not the
   ...
   the carve-out did not leak onto the remaining climber-archetype
   townsfolk who still share the plain Hiker/Miner/Ranger/Mountain Guide/
   Caver/Cave Guide classes (Flint, Moss, Roan, Geode, Nils, Sump, and the
   Calculus-region Dov/Orin/Ruben/Tovah). Neither Burl nor Crag has a
   personal badge gate, so both use the standard Tier 2 badge floor (2/5),
   same as most prior batches. */
const { chromium } = require('./playwright.cjs');

const CHARACTERS = [
  { id: 'burl', archetype: 'ballast', item: 'pressedFlower', otherId: 'crag',
    eventIds: ['burl-folk-ballast-event-the-full-pack', 'burl-folk-ballast-event-setting-the-kettle-down'],
    firstAcknowledgePool: 'burl-dialogue-post-the-full-pack', replayTitle: 'The full pack' },
  { id: 'crag', archetype: 'stride', item: 'carvedWhistle', otherId: 'burl',
    eventIds: ['crag-folk-stride-event-the-same-step', 'crag-folk-stride-event-more-than-one-step'],
    firstAcknowledgePool: 'crag-dialogue-post-the-same-step', replayTitle: 'The same step' }
];
...
(async () => {
  ...
  for (const person of CHARACTERS) {
    const data = await page.evaluate(({ id, item, eventIds }) => {
      ...
      const contexts = [
        ...
        selectCharacterDialogue(id, { location: 'ridge', mood: 'neutral' }),
  ...
  const crossRef = await page.evaluate(() => {
    const burlLines = (CORE_CAST_DIALOGUE.burl || []).flatMap(pool => pool.lines).join(' ');
    const cragLines = (CORE_CAST_DIALOGUE.crag || []).flatMap(pool => pool.lines).join(' ');
    const ballastEventText = (window.FOLK_EVENTS['ballast'] || []).flatMap(event => event.beats.map(beat => beat.s)).join(' ');
    const strideEventText = (window.FOLK_EVENTS['stride'] || []).flatMap(event => event.beats.map(beat => beat.s)).join(' ');
    return {
      burlMentionsCrag: /\bCrag\b/.test(burlLines) && /\bCrag\b/.test(ballastEventText),
      cragMentionsBurl: /\bBurl\b/.test(cragLines) && /\bBurl\b/.test(strideEventText)
    };
  });
  check('Burl\'s dialogue and event text reference Crag by name (ordinary narrative cross-reference, not the relationshipIds gate)', crossRef.burlMentionsCrag);
  check('Crag\'s dialogue and event text reference Burl by name (ordinary narrative cross-reference, not the relationshipIds gate)', crossRef.cragMentionsBurl);

  const isolation = await page.evaluate(() => {
    /* geo/null/leak were still plain-climber when this suite was written (Slice
       6), but Slice 7 carved them out too (see check-cavern-keepers-phase5.cjs) -
       excluded here so this list only asserts about members still actually
       sharing the plain climber archetype today. */
  ...
  check('the carve-out did not leak onto the remaining plain climber-family townsfolk (still 3 events each)',
    isolation.every(r => r.cls !== 'Loaded Hiker' && r.cls !== 'Steady Hiker' && r.arcKey === 'climber' && r.eventCount === 3),
